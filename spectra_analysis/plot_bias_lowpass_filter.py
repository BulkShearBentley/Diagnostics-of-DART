...
variables = {
    ...
    "Specific_Humidity_BIAS": {
        "label": "Specific Humidity Bias",
        "units": "kg kg$^{-1}$"
    },
    "U_wind_BIAS": {
        "label": "U Wind Bias",
...
output_dir = "bias_timeseries_plots"
...
for exp in experiments:
    ...
    for file in files:
        ...
        timestamp_str = file.split("_BIAS_")[-1].replace(".nc", "")
...
for ilev, pressure in enumerate(pressure_levels):
    ...
    fig.suptitle(
        f"Bias Time Series at Pressure Level = {pressure/100:.0f} hPa",
        fontsize=16
    )
    ...
    output_file = os.path.join(
        output_dir,
        f"BIAS__timeseries_{int(pressure/100)}hPa.png"
    )
